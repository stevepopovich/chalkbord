...
var UserSignUpComponent = (function () {
    function UserSignUpComponent(formBuilder, auth, viewControl, toastCtrl, deviceService) {
        ...
        this.deviceService.getUserEmailPasswordFromLocalStorage().then(function (emailPasswordTup) {
            if (emailPasswordTup) {
                _this.userFormGroup.get("email").setValue(emailPasswordTup.email);
                _this.userFormGroup.get("password").setValue(emailPasswordTup.password);
                _this.login();
            }
        });
    }
