...
var DealEditorComponent = (function () {
    ...
    DealEditorComponent.prototype.save = function () {
        ...
        if (this.dealEditorFormGroup.valid) {
            this.uploader.captureImage().then(function (imageData) {
                var deal;
                if (!_this.limitDealNumber) {
                    deal = new __WEBPACK_IMPORTED_MODULE_2__types_deals_type__["a" /* DealModel */](new __WEBPACK_IMPORTED_MODULE_2__types_deals_type__["c" /* RestaurantModel */]("Name1", "Columbus, OH", ""), _this.dealEditorFormGroup.get("dealDescription").value, new Date(_this.dealEditorFormGroup.get("dealStart").value), new Date(_this.dealEditorFormGroup.get("dealEnd").value), -1, _this.dealEditorFormGroup.get("dealType").value, null);
                    deal.imageSource = "/locale-deal-photos/" + deal.id;
                }
                else {
                    deal = new __WEBPACK_IMPORTED_MODULE_2__types_deals_type__["a" /* DealModel */](new __WEBPACK_IMPORTED_MODULE_2__types_deals_type__["c" /* RestaurantModel */]("Name1", "Columbus, OH", ""), _this.dealEditorFormGroup.get("dealDescription").value, new Date(_this.dealEditorFormGroup.get("dealStart").value), new Date(_this.dealEditorFormGroup.get("dealEnd").value), _this.dealEditorFormGroup.get("dealNumber").value, _this.dealEditorFormGroup.get("dealType").value, null);
                    deal.imageSource = "/locale-deal-photos/" + deal.id;
                }
                _this.uploader.uploadDealPhoto(imageData, deal.id);
