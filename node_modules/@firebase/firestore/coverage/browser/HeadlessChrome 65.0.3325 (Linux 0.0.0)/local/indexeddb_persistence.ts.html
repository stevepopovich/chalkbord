...
<html lang="en">
<head>
    <title>Code coverage report for local/indexeddb_persistence.ts</title>
    <meta charset="utf-8" />
...
<div class='wrapper'>
  <div class='pad1'>
    <h1>
      <a href="../index.html">All files</a> / <a href="index.html">local</a> indexeddb_persistence.ts
    </h1>
    <div class='clearfix'>
      <div class='fl pad1y space-right2'>
        <span class="strong">74.5% </span>
        <span class="quiet">Statements</span>
        <span class='fraction'>111/149</span>
      </div>
      <div class='fl pad1y space-right2'>
      ...
      <div class='fl pad1y space-right2'>
        <span class="strong">72.66% </span>
        <span class="quiet">Lines</span>
        <span class='fraction'>101/139</span>
      </div>
    </div>
  </div>
...
417
418
419</td><td class="line-coverage quiet"><span class="cline-any cline-neutral">&nbsp;</span>
...
<span class="cline-any cline-neutral">&nbsp;</span>
<span class="cline-any cline-neutral">&nbsp;</span>
<span class="cline-any cline-yes">6154x</span>
<span class="cline-any cline-neutral">&nbsp;</span>
<span class="cline-any cline-neutral">&nbsp;</span>
<span class="cline-any cline-neutral">&nbsp;</span>
<span class="cline-any cline-yes">6154x</span>
...
&nbsp;
  start(): Promise&lt;void&gt; {
    <span class="missing-if-branch" title="if path not taken" >I</span>if (!IndexedDbPersistence.isAvailable()) {
<span class="cstat-no" title="statement not covered" >      this.persistenceError = new FirestoreError(</span>
        Code.UNIMPLEMENTED,
        UNSUPPORTED_PLATFORM_ERROR_MSG
      );
<span class="cstat-no" title="statement not covered" >      return Promise.reject(this.persistenceError);</span>
    }
&nbsp;
...
&nbsp;
  runTransaction&lt;T&gt;(
    ...
    <span class="missing-if-branch" title="if path not taken" >I</span>if (this.persistenceError) {
<span class="cstat-no" title="statement not covered" >      return Promise.reject(this.persistenceError);</span>
    }
&nbsp;
...
&nbsp;
  ...
  static buildStoragePrefix(databaseInfo: DatabaseInfo): string {
    ...
    <span class="missing-if-branch" title="if path not taken" >I</span>if (!databaseInfo.databaseId.isDefaultDatabase) {
<span class="cstat-no" title="statement not covered" >      database += '.' + databaseInfo.databaseId.database;</span>
    }
&nbsp;
  ...
  private tryAcquireOwnerLease(): Promise&lt;void&gt; {
    ...
    return this.simpleDb.runTransaction('readwrite', [DbOwner.store], txn =&gt; {
      ...
      return store.get('owner').next(dbOwner =&gt; {
        <span class="missing-if-branch" title="else path not taken" >E</span>if (!this.validOwner(dbOwner)) {
...
<span class="cstat-no" title="statement not covered" >          return PersistencePromise.reject&lt;void&gt;(this.persistenceError);</span>
        }
      });
    });
  }
&nbsp;
  ...
  private releaseOwnerLease(): Promise&lt;void&gt; {
    ...
    return this.simpleDb.runTransaction('readwrite', [DbOwner.store], txn =&gt; {
      const store = txn.store&lt;DbOwnerKey, DbOwner&gt;(DbOwner.store);
      return store.get('owner').next(dbOwner =&gt; {
        <span class="missing-if-branch" title="else path not taken" >E</span>if (dbOwner !== null &amp;&amp; dbOwner.ownerId === this.ownerId) {
          log.debug(LOG_TAG, 'Releasing owner lease.');
          return store.delete('owner');
        } else {
<span class="cstat-no" title="statement not covered" >          return PersistencePromise.resolve();</span>
        }
      });
    });
  }
&nbsp;
  ...
  private ensureOwnerLease(txn: SimpleDbTransaction): PersistencePromise&lt;void&gt; {
    const store = txn.store&lt;DbOwnerKey, DbOwner&gt;(DbOwner.store);
    return store.get('owner').next(dbOwner =&gt; {
      <span class="missing-if-branch" title="if path not taken" >I</span>if (dbOwner === null || dbOwner.ownerId !== this.ownerId) {
<span class="cstat-no" title="statement not covered" >        this.persistenceError = new FirestoreError(</span>
          Code.FAILED_PRECONDITION,
          EXISTING_OWNER_ERROR_MSG
        );
...
&nbsp;
  ...
  private validOwner(dbOwner: DbOwner | null): boolean {
    ...
    <span class="missing-if-branch" title="else path not taken" >E</span>if (dbOwner === null) {
      return false; // no owner.
    } else <span class="cstat-no" title="statement not covered" >if (dbOwner.leaseTimestampMs &lt; minAcceptable) {</span>
<span class="cstat-no" title="statement not covered" >      return false; </span>// owner lease has expired.
    } else <span class="cstat-no" title="statement not covered" >if (dbOwner.leaseTimestampMs &gt; maxAcceptable) {</span>
...
<span class="cstat-no" title="statement not covered" >      return false;</span>
    } else <span class="cstat-no" title="statement not covered" >if (dbOwner.ownerId === this.getZombiedOwnerId()) {</span>
<span class="cstat-no" title="statement not covered" >      return false; </span>// owner's tab closed.
    } else {
...
&nbsp;
  ...
  private scheduleOwnerLeaseRefreshes(): void {
    ...
    this.ownerLeaseRefreshHandle = setInterval(<span class="fstat-no" title="function not covered" >() =&gt; {</span>
      const txResult = <span class="cstat-no" title="statement not covered" >this.runTransaction('Refresh owner timestamp', <span class="fstat-no" title="function not covered" >txn =&gt; {</span></span>
        // NOTE: We don't need to validate the current owner contents, since
        // runTransaction does that automatically.
        const store = <span class="cstat-no" title="statement not covered" >txn.store&lt;DbOwnerKey, DbOwner&gt;(DbOwner.store);</span>
<span class="cstat-no" title="statement not covered" >        return store.put('owner', new DbOwner(this.ownerId, Date.now()));</span>
      });
&nbsp;
<span class="cstat-no" title="statement not covered" >      txResult.catch(<span class="fstat-no" title="function not covered" >reason =&gt; {</span></span>
        // Probably means we lost the lease. Report the error and stop trying to
        // refresh the lease.
<span class="cstat-no" title="statement not covered" >        log.error(reason);</span>
<span class="cstat-no" title="statement not covered" >        this.stopOwnerLeaseRefreshes();</span>
      });
    }, OWNER_LEASE_REFRESH_INTERVAL_MS);
  }
&nbsp;
  private stopOwnerLeaseRefreshes(): void {
    <span class="missing-if-branch" title="else path not taken" >E</span>if (this.ownerLeaseRefreshHandle) {
...
&nbsp;
  ...
  private attachWindowUnloadHook(): void {
    this.windowUnloadHandler = <span class="fstat-no" title="function not covered" >() =&gt; {</span>
      // Record that we're zombied.
<span class="cstat-no" title="statement not covered" >      this.setZombiedOwnerId(this.ownerId);</span>
&nbsp;
      // Attempt graceful shutdown (including releasing our owner lease), but
      // there's no guarantee it will complete.
<span class="cstat-no" title="statement not covered" >      this.shutdown();</span>
    };
    window.addEventListener('unload', this.windowUnloadHandler);
  }
&nbsp;
  private detachWindowUnloadHook(): void {
    <span class="missing-if-branch" title="else path not taken" >E</span>if (this.windowUnloadHandler) {
...
&nbsp;
   ...
   * record exists.
   */
<span class="fstat-no" title="function not covered" >  private getZombiedOwnerId(): string | null {</span>
<span class="cstat-no" title="statement not covered" >    try {</span>
      const zombiedOwnerId = <span class="cstat-no" title="statement not covered" >window.localStorage.getItem(</span>
        this.zombiedOwnerLocalStorageKey()
      );
<span class="cstat-no" title="statement not covered" >      log.debug(LOG_TAG, 'Zombied ownerID from LocalStorage:', zombiedOwnerId);</span>
<span class="cstat-no" title="statement not covered" >      return zombiedOwnerId;</span>
    } catch (e) {
      // Gracefully handle if LocalStorage isn't available / working.
<span class="cstat-no" title="statement not covered" >      log.error('Failed to get zombie owner id.', e);</span>
...
&nbsp;
   ...
   */
<span class="fstat-no" title="function not covered" >  private setZombiedOwnerId(</span>zombieOwnerId: string | null) {
<span class="cstat-no" title="statement not covered" >    try {</span>
<span class="cstat-no" title="statement not covered" >      if (zombieOwnerId === null) {</span>
<span class="cstat-no" title="statement not covered" >        window.localStorage.removeItem(this.zombiedOwnerLocalStorageKey());</span>
      } else {
<span class="cstat-no" title="statement not covered" >        window.localStorage.setItem(</span>
      ...
      // Gracefully handle if LocalStorage isn't available / working.
<span class="cstat-no" title="statement not covered" >      log.error('Failed to set zombie owner id.', e);</span>
    }
  }
&nbsp;
<span class="fstat-no" title="function not covered" >  private zombiedOwnerLocalStorageKey(): string {</span>
<span class="cstat-no" title="statement not covered" >    return this.localStoragePrefix + ZOMBIE_OWNER_LOCALSTORAGE_SUFFIX;</span>
  }
&nbsp;
